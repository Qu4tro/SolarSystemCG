...
void create_sphere2(FILE* f, float radius, int slices, int stacks){
    ...
    for (i = 0; i < stacks; i++) {
        for(j = 0; j < slices; j++) {
            ...
            v1X = radius * sin(alpha + sliceAngle) * cos(beta);
            v1Y = radius * sin(beta);
            v1Z = radius * cos(alpha + sliceAngle) * cos(beta);

            v2X = radius * sin(alpha) * cos(beta + stackAngle);
            v2Y = radius * sin(beta + stackAngle);
            v2Z = radius * cos(alpha) * cos(beta + stackAngle);
            ...
            add_point(f,
                    radius * sin(alpha + sliceAngle) * cos(beta + stackAngle),
                    radius * sin(beta + stackAngle),
                    radius * cos(alpha + sliceAngle) * cos(beta + stackAngle));
            add_point(f, v2X, v2Y, v2Z);
            add_point(f, v1X, v1Y, v1Z);
        }
    }
